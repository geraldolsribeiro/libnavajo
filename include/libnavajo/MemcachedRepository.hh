...
public:
  MemcachedRepository(const string &prefix, const string &server = "127.0.0.1",
                      const int port = 11211);
  virtual ~MemcachedRepository() {};
  bool set(const string &url, const vector<char> &vec, time_t expiry = 0,
           uint32_t flags = 0);
  bool set(const string &url, const string &value, time_t expiry = 0,
           uint32_t flags = 0);
